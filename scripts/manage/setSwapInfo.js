...
import swapperAbi from "../../artifacts/contracts/PRFCT/infra/PrfctSwapper.sol/PrfctSwapper.json";
...
const prfctfinanceSwapper = "0x3Ca6e5cf70bCf11cB8C73BBa13C55DFb91334B11";
...
async function balancer() {
  ...
  const swapSteps = [];
  const assets = [];
  const funds = [prfctfinanceSwapper, false, prfctfinanceSwapper, false];
...
async function solidly() {
  ...
  const txData = await router.populateTransaction.swapExactTokensForTokens(3, 4, config.solidly.path, prfctfinanceSwapper, uint256Max);
  const amountIndex = 4;
  const minIndex = 36;
  const minAmountSign = 0;
...
async function setSwapInfo(fromToken, toToken, swapInfo) {
  const swapper = await ethers.getContractAt(swapperAbi.abi, prfctfinanceSwapper);
