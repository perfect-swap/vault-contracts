...
import { PrfctHarvester, UpkeepRefunder } from "../../typechain-types";
...
const config = {
  harvester: {
    contractName: "PrfctHarvester",
    args: {
      vaultRegistry: chainData.platforms.prfctfinance.vaultRegistry,
      keeperRegistry,
...
const deployHarvester = async (upkeepAddress: string) => {
  const PrfctHarvesterFactory = await ethers.getContractFactory(config.harvester.contractName);
