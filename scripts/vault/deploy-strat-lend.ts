...
import { PrfctChain } from "../../utils/prfctChain";

const {
  platforms: { spookyswap, spiritswap, prfctfinance },
  tokens: {
    TUSD: { address: TUSD },
    FTM: {address: FTM},
    USDC: { address: USDC },
...
async function main() {
  ...
  await setCorrectCallFee(strategy, hardhat.network.name as PrfctChain);
  console.log();

 // console.log(`Transfering Vault Owner to ${prfctfinance.vaultOwner}`)
 // await vault.transferOwnership(prfctfinance.vaultOwner);
 // console.log();
