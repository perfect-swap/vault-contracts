...
async function main() {
  ...
  const contractNames = {
    prfctStakedFantom: "PrfctEscrowedFantom",
  };

  const PrfctStakedFantom = await ethers.getContractFactory(contractNames.prfctStakedFantom);

  console.log('deploying Prfct Staked Fantom Contract');
  ...
  const staker = await PrfctStakedFantom.deploy(...lockerArguments);
