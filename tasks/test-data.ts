...
task("test-data:addressbook:prfct", "Fetch prfct addressbook to inject platform addresses in forge tests")
  ...
  .setAction(async (taskArgs: { chain: keyof typeof addressBook}, hre, runSuper) => {
    ...
    const { prfctfinance } = addressBook[taskArgs.chain].platforms;
    const data = {
      keeper: prfctfinance.keeper,
      strategyOwner: prfctfinance.strategyOwner,
      vaultOwner: prfctfinance.vaultOwner,
    };
