...
const config = {
  ...
  timelock: addressBook[chainName].platforms.prfctfinance.vaultOwner,
};
...
describe("StratUpgrade", function () {
  ...
  before(async () => {
    ...
    vault = await ethers.getContractAt("PrfctVaultV6", config.vault);
  ...
  it("New strat has the correct admin accounts", async () => {
    const { prfctfinance } = addressBook[chainName].platforms;
    expect(await candidate.keeper()).to.equal(prfctfinance.keeper);
    expect(await candidate.owner()).to.equal(prfctfinance.strategyOwner);
  });
