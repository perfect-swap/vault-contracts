...
contract StrategistBuyback is OwnableUpgradeable {
    ...
    function initialize(
        ...
        prfctMaxi = _prfctMaxi;
        unirouter = _unirouter;

        _setNativeToWantRoute(_nativeToWantRoute);

        IERC20_StrategistBuyback(native).approve(unirouter, type(uint256).max);
        // approve spending by prfctMaxi
        IERC20_StrategistBuyback(native).approve(prfctMaxi, type(uint256).max);
        IERC20_StrategistBuyback(want).approve(prfctMaxi, type(uint256).max);

        trackVault(address(0)); // dummy vault to overcome issue where mapping values are defaulted to 0;
    }

    function depositVaultWantIntoPrfctMaxi() external onlyOwner {
        _depositVaultWantIntoPrfctMaxi();
    }

    function withdrawVaultWantFromPrfctMaxi() external onlyOwner {
        _withdrawVaultWantFromPrfctMaxi();
    }

    // Convert and send to prfct maxi
    ...
    function _depositVaultWantIntoPrfctMaxi() internal {
        IVault_StrategistBuyback(prfctMaxi).depositAll();
    }

    function _withdrawVaultWantFromPrfctMaxi() internal {
        IVault_StrategistBuyback(prfctMaxi).withdrawAll();
    }
    ...
    function balanceOfMooTokens() public view returns (uint256) {
        return IERC20_StrategistBuyback(prfctMaxi).balanceOf(address(this));
    }
