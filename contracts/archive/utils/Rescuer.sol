...
// Built to safely flash loan amount needed to upgrade Prfct Vault to new strategy 
contract Rescuer is Ownable {
    ...
    IPrfctVault public prfctVault = IPrfctVault(0x42ECfA11Db08FB3Bb0AAf722857be56FA8E57Dc0);
    ...
    function tusdNeeded() public view returns (uint256) {
        return prfctVault.balance() - tusd.balanceOf(address(iToken));
    }
    ...
    function upgrade() external onlyOwner {
        ...
        IStrategy(prfctVault.strategy()).updateBalance();
        ...
        balancerVault.flashLoan(address(this), tokens, amounts, "PrfctBalancerFlash");
    }
    ...
    function receiveFlashLoan(address[] memory, uint256[] memory amounts, uint256[] memory feeAmounts, bytes memory) external {
        tusdPair.swap(0, tusdNeeded() - amounts[0], address(this), "PrfctRescue");
        uint256 balancerDebt = amounts[0] + feeAmounts[0];
        tusd.safeTransfer(address(balancerVault), balancerDebt);
    }

    function transferPrfctVaultOwnership(address _newOwner) external onlyOwner { 
        emit VaultOwnershipChanged(prfctVault.owner(), _newOwner);
        prfctVault.transferOwnership(_newOwner);
    }
