...
contract StrategyDFYNRewardPoolLP is StratManager, FeeManager {
    ...
    // Routes
    address[] public outputToNativeRoute; // since DFYN uses its own native, convert to common token, then convert that token to native prfct uses
    ...
    function chargeFees() internal {
        ...
        uint256 prfctFeeAmount = wmaticBal.mul(prfctFee).div(MAX_FEE);
        IERC20(wmatic).safeTransfer(prfctFeeRecipient, prfctFeeAmount);
