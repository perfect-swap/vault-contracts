...
interface IPrfctVault is IERC20 {
    function deposit(uint256 amount) external;
    function withdraw(uint256 shares) external;
    function want() external pure returns (address); // Prfct Vault V6
    function token() external pure returns (address); // Prfct Vault V5
}

contract PrfctZapMdex {
    ...
    function beefOutAndSwap(address prfctVault, uint256 withdrawAmount, address desiredToken, uint256 desiredTokenOutMin) external {
        ...
        require(token0 == desiredToken || token1 == desiredToken, 'Prfct: desired token not present in liquidity pair');
    ...
    function _getVaultPair (address prfctVault) private view returns (IPrfctVault vault, IUniswapV2Pair pair) {
    ...
    function _swapAndStake(address prfctVault, uint256 tokenAmountOutMin, address tokenIn) private {
        ...
        require(isInputA || pair.token1() == tokenIn, 'Prfct: Input token not present in liquidity pair');
