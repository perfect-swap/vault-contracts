...
// @author Wivern & Weso for Prfct.Finance
...
contract PrfctZapSolidlyDyst {
    ...
    function beefOut(address prfctVault, uint256 withdrawAmount) external {
        (IPrfctVaultV6 vault, ISolidlyPair pair) = _getVaultPair(prfctVault);
    ...
    function _getVaultPair(address prfctVault) private pure returns (IPrfctVaultV6 vault, ISolidlyPair pair) {
    ...
    function estimateSwap(address prfctVault, address tokenIn, uint256 fullInvestmentIn) public view returns(uint256 swapAmountIn, uint256 swapAmountOut, address swapTokenOut) {
        checkWETH();
        (, ISolidlyPair pair) = _getVaultPair(prfctVault);
