...
import "./zapInterfaces/IPrfctVault.sol";
import "./zapInterfaces/IStrategy.sol";
import "./zapInterfaces/IERC20Extended.sol";


// Aggregator Zap compatible with all single asset, uniswapv2, and solidly router Prfct Vaults. 
contract PrfctZapOneInch {
    ...
    function beefInETH (address _prfctVault, bytes calldata _token0, bytes calldata _token1, WantType _type) external payable {
        require(msg.value >= minimumAmount, 'Prfct: Insignificant input amount');

        IWETH(WETH).deposit{value: msg.value}();
        _swapAndStake(_prfctVault, WETH, WETH, _token0, _token1, _type);
        emit ZapIn(_prfctVault, WETH, msg.value);
    }

    function beefIn (address _prfctVault, address _inputToken, uint256 _tokenInAmount, bytes calldata _token0, bytes calldata _token1, WantType _type) public {
        require(_tokenInAmount >= minimumAmount, 'Prfct: Insignificant input amount');

        IERC20(_inputToken).safeTransferFrom(msg.sender, address(this), _tokenInAmount);
        _swapAndStake(_prfctVault, _inputToken, _inputToken, _token0, _token1, _type);
        emit ZapIn(_prfctVault,  _inputToken, _tokenInAmount);
    }

    function beefOut (address _prfctVault, uint256 _withdrawAmount) external {
        address[] memory tokens = _beefOut(_prfctVault, _withdrawAmount);
         _returnAssets(tokens);
    }

    function beefOutAndSwap(address _prfctVault, uint256 _withdrawAmount, address _desiredToken, bytes calldata _dataToken0, bytes calldata _dataToken1, WantType _type) external {
        (IPrfctVault vault, IUniswapV2Pair pair) =  _getVaultPair(_prfctVault);
        vault.safeTransferFrom(msg.sender, address(this), _withdrawAmount);
        vault.withdraw(_withdrawAmount);
         emit ZapOut(_prfctVault, _desiredToken, _withdrawAmount);
    ...
    function beefOutAndReInvest(
        ...
        (IPrfctVault vault, IUniswapV2Pair pair) = _getVaultPair(_fromMooVault);
    ...
    function quoteStableAddLiquidityRatio(address _prfctVault) external view returns (uint256 ratio1to0) {
            (IPrfctVault vault, IUniswapV2Pair pairAddress) = _getVaultPair(_prfctVault);
    ...
    // Internal functions
    function _beefOut (address _prfctVault, uint256 _withdrawAmount) private returns (address[] memory tokens) {
        (IPrfctVault vault, IUniswapV2Pair pair) = _getVaultPair(_prfctVault);

        IERC20(_prfctVault).safeTransferFrom(msg.sender, address(this), _withdrawAmount);
        vault.withdraw(_withdrawAmount);
        ...
        emit ZapOut(_prfctVault, address(pair), _withdrawAmount);
    }
    ...
    function _getVaultPair (address _prfctVault) private pure returns (IPrfctVault vault, IUniswapV2Pair pair) {
        vault = IPrfctVault(_prfctVault);
    ...
    function _swapAndStake(address _vault, address _inputToken, bytes calldata _token0) private {
        IPrfctVault vault = IPrfctVault(_vault);
        address[] memory path;
        path = new address[](2);
        path[0] = vault.want();
        path[1] = _inputToken;
    ...
    function _swapAndStake(address _prfctVault, address _inputToken0, address _inputToken1, bytes calldata _token0, bytes calldata _token1, WantType _type) private {
        (IPrfctVault vault, IUniswapV2Pair pair) =  _getVaultPair(_prfctVault);
        ...
        if (_type != WantType.WANT_TYPE_SINGLE) {
            ...
            _swapAndStake(_prfctVault, _inputToken0, _token0);
        }
    }
