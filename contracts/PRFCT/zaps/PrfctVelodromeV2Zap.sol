...
contract PrfctVelodromeV2Zap {
    ...
    constructor(address _router, address _WETH) {
        ...
        require(WETH == router.weth(), 'Prfct: WETH address not matching Router.weth()');
    }
    ...
    function beefOut (address prfctVault, uint256 withdrawAmount) external {
        (IPrfctVault vault, IPool pair) = _getVaultPair(prfctVault);
    ...
    function _getVaultPair (address prfctVault) private pure returns (IPrfctVault vault, IPool pair) {
    ...
    function _swapAndStake(address prfctVault, uint256 tokenAmountOutMin, address tokenIn) private {
        (IPrfctVault vault, IPool pair) = _getVaultPair(prfctVault);
        require(pair.factory() == router.defaultFactory(), 'Prfct: Incompatible liquidity pair'); // router.addLiquidity adds to pair from router.defaultFactory()
    ...
    function estimateSwap(address prfctVault, address tokenIn, uint256 fullInvestmentIn) public view returns (uint256 swapAmountIn, uint256 swapAmountOut, address swapTokenOut) {
        (, IPool pair) = _getVaultPair(prfctVault);
        require(pair.factory() == router.defaultFactory(), 'Prfct: Incompatible liquidity pair'); // router.addLiquidity adds to pair from router.defaultFactory()
