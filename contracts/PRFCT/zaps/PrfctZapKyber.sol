...
contract PrfctZapKyber {
    ...
    function beefInETH (address prfctVault, bytes memory token0, bytes memory token1) external payable {
        require(msg.value >= minimumAmount, 'Prfct: Insignificant input amount');

        IWETH(WETH).deposit{value: msg.value}();
        _swapAndStake(prfctVault, WETH, WETH, token0, token1);
    }

    function beefIn (address prfctVault, address inputToken, uint256 tokenInAmount, bytes memory token0, bytes memory token1) public {
        require(tokenInAmount >= minimumAmount, 'Prfct: Insignificant input amount');
        require(IERC20(inputToken).allowance(msg.sender, address(this)) >= tokenInAmount, 'Prfct: Input token is not approved');

        IERC20(inputToken).safeTransferFrom(msg.sender, address(this), tokenInAmount);

        _swapAndStake(prfctVault, inputToken, inputToken, token0, token1);
    }

    function beefOut (address prfctVault, uint256 withdrawAmount) external {
        address[] memory tokens = _beefOut(prfctVault, withdrawAmount);
         _returnAssets(tokens);
    }

    function _beefOut (address prfctVault, uint256 withdrawAmount) internal returns (address[] memory tokens) {
        (IPrfctVault vault, IUniswapV2Pair pair) = _getVaultPair(prfctVault);
    ...
    function beefOutAndSwap(address prfctVault, uint256 withdrawAmount, address desiredToken, bytes memory dataToken0, bytes memory dataToken1) external {
        (IPrfctVault vault, IUniswapV2Pair pair) = _getVaultPair(prfctVault);
    ...
    function quoteStableAddLiquidityRatio(IPrfctVault prfctVault) external view returns (uint256 ratio0to1) {
            (,IUniswapV2Pair pairAddress) = _getVaultPair(address(prfctVault));
            ...
            ISolidlyRouter router = ISolidlyRouter(IStrategy(prfctVault.strategy()).unirouter());
    ...
    function quoteRemoveLiquidity(IPrfctVault prfctVault, uint256 mooTokenAmt) external view returns (uint256 amt0, uint256 amt1, address token0, address token1) {
        uint256 withdrawFee = IStrategy(prfctVault.strategy()).withdrawalFee();
        uint256 liquidity = mooTokenAmt * prfctVault.balance() / prfctVault.totalSupply();
        uint256 fee = withdrawFee > 0 ? liquidity * withdrawFee / 10000 : 0;
        liquidity = liquidity - fee;
        
        (, IUniswapV2Pair pair) = _getVaultPair(address(prfctVault));
    ...
    function _getVaultPair (address prfctVault) private pure returns (IPrfctVault vault, IUniswapV2Pair pair) {
        vault = IPrfctVault(prfctVault);
    ...
    function _swapAndStake(address prfctVault, address inputToken0, address inputToken1, bytes memory token0, bytes memory token1) private {
        (IPrfctVault vault, IUniswapV2Pair pair) = _getVaultPair(prfctVault);
