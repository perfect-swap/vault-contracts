...
/// @title Prfct fee batch
/// @author kexley, Prfct
/// @notice All Prfct fees will flow through to the treasury and the reward pool
/// @dev Wrapped ETH will build up on this contract and will be swapped via the Prfct Swapper to
/// the pre-specified tokens and distributed to the treasury and reward pool
contract PrfctFeeBatchV4 is OwnableUpgradeable {
    ...
    function _distributeTreasuryFee() private {
        ...
        for (uint i; i < treasuryTokens.tokens.length; ++i) {
            ...
            if (token != address(native)) {
                amount = IPrfctSwapper(swapper).swap(address(native), token, amount);
                if (amount == 0) continue;
            }
    ...
    function _notifyRewardPool() private {
        ...
        for (uint i; i < rewardTokens.tokens.length; ++i) {
            ...
            IPrfctRewardPool(rewardPool).notifyRewardAmount(token, amount, duration);
