...
contract PrfctFeeBatchV3SolidlyRouter is Initializable, OwnableUpgradeable {
    ...
    ISolidlyRouter.Routes[] public wNativeToPrfctRoute;
    ...
    function initialize(
        ...
        address _rewardPool, 
        address _unirouter,
        ISolidlyRouter.Routes[] memory _prfctRoute,
        ...
        for (uint i; i < _prfctRoute.length; ++i) {
                wNativeToPrfctRoute.push(_prfctRoute[i]);
            }
    ...
    function harvest() public {
        ...
        if (splitTreasury) {
            ...
            ISolidlyRouter(unirouter).swapExactTokensForTokens(treasuryHalf, 0, wNativeToPrfctRoute, treasury, block.timestamp);
        } else {
    ...
    function setRoute(ISolidlyRouter.Routes[] memory _route, bool _stableRoute) external onlyOwner {
        ...
        if(_stableRoute) {
            ...
            delete wNativeToPrfctRoute;
            require(_route[_route.length - 1].to == address(prfct), "!PRFCT");
             for (uint i; i < _route.length; ++i) {
                wNativeToPrfctRoute.push(_route[i]);
            }
        }
    }
