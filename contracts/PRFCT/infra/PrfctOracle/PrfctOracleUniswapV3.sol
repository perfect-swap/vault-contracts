...
import { PrfctOracleHelper, IPrfctOracle, PrfctOracleErrors } from "./PrfctOracleHelper.sol";

/// @title Prfct Oracle for UniswapV3
/// @author Prfct, @kexley
/// @notice On-chain oracle using UniswapV3
contract PrfctOracleUniswapV3 {
    ...
    function getPrice(bytes calldata _data) external returns (uint256 price, bool success) {
        ...
        price = PrfctOracleHelper.priceFromBaseToken(
    ...
    function validateData(bytes calldata _data) external view {
        ...
        if (tokens.length != pools.length + 1 || tokens.length != twapPeriods.length + 1) {
            revert PrfctOracleErrors.ArrayLength();
        }
        
        uint256 basePrice = IPrfctOracle(msg.sender).getPrice(tokens[0]);
        if (basePrice == 0) revert PrfctOracleErrors.NoBasePrice(tokens[0]);
        ...
        for (uint i; i < poolLength;) {
            ...
            if (fromToken != token0 && fromToken != token1) {
                revert PrfctOracleErrors.TokenNotInPair(fromToken, pool);
            }
            if (toToken != token0 && toToken != token1) {
                revert PrfctOracleErrors.TokenNotInPair(toToken, pool);
            }
            unchecked { ++i; }
        }
    }
