...
/// @title Prfct Oracle for UniswapV2
/// @author Prfct, @kexley
    ...
    function validateData(bytes calldata _data) external view {
        ...
        for (uint i; i < pairLength;) {
            ...
            if (fromToken != token0 && fromToken != token1) {
                revert PrfctOracleErrors.TokenNotInPair(fromToken, pair);
            }
            if (toToken != token0 && toToken != token1) {
                revert PrfctOracleErrors.TokenNotInPair(toToken, pair);
            }
            unchecked { ++i; }
        }
    }
