...
/// @title Reward pool for PRFCT
/// @author kexley, Prfct
/// @notice Multi-reward staking contract for PRFCT
/// @dev Multiple rewards can be added to this contract by the owner. A receipt token is issued for 
/// staking and is used for withdrawing the staked PRFCT.
contract PrfctRewardPool is ERC20Upgradeable, OwnableUpgradeable {
    ...
    /// @notice Withdraw PRFCT tokens
    /// @dev Burns an equal number of receipt tokens from the caller
    /// @param _amount Amount of PRFCT to withdraw
    function withdraw(uint256 _amount) external update(msg.sender) {
        _withdraw(_amount);
    }

    /// @notice Withdraw all of the caller's PRFCT tokens and claim rewards
    ...
    /// @dev Stake PRFCT tokens and mint the caller receipt tokens
    ...
    /// @dev Withdraw PRFCT tokens and burn an equal number of receipt tokens from the caller
    /// @param _amount Amount of PRFCT to withdraw
    ...
    /// @dev Calculate the reward amount per PRFCT token
    /// @param _reward Address of the reward
    /// @return rewardPerToken Reward amount per PRFCT token
