...
import "../interfaces/IPrfctVault.sol";
import "../interfaces/IPrfctStrategy.sol";
import "../interfaces/IPrfctRegistry.sol";
import "../interfaces/IPrfctHarvester.sol";
import "../interfaces/IUpkeepRefunder.sol";

import "../libraries/UpkeepLibrary.sol";

contract PrfctHarvester is ManageableUpgradeable, IPrfctHarvester {
    using SafeERC20Upgradeable for IERC20Upgradeable;

    // Contracts.
    IPrfctRegistry public _vaultRegistry;
    ...
    uint256 public _vaultHarvestFunctionGasOverhead; // Estimated average gas cost of calling harvest(). TODO: this needs to live in PrfctRegistry, and needs to be a `per vault` number.
    ...
    function initialize(
        ...
        _vaultRegistry = IPrfctRegistry(vaultRegistry_);
        ...
        )
    {
        ...
        if (numberOfVaultsToHarvest == 0) return (false, bytes("PrfctAutoHarvester: No vaults to harvest"));
    ...
    function _canHarvestVault(address vaultAddress_) internal view returns (bool canHarvest_) {
        IPrfctVault vault = IPrfctVault(vaultAddress_);
        IPrfctStrategy strategy = IPrfctStrategy(vault.strategy());
        ...
        )
    {
        ...
        uint256 vaultHarvestGasOverhead = _estimateSingleVaultHarvestGasOverhead(_vaultHarvestFunctionGasOverhead); // TODO: Pull this number from PrfctRegistry.
    ...
    function _harvestVault(address vault_) internal returns (bool didHarvest_, uint256 callRewards_) {
        IPrfctStrategy strategy = IPrfctStrategy(IPrfctVault(vault_).strategy());
