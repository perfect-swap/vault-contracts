...
contract PrfctRegistry is ManageableUpgradeable, IPrfctRegistry {
    ...
    function _addVault(address _vaultAddress) internal {
        ...
        IPrfctVault vault = IPrfctVault(_vaultAddress);
        IPrfctStrategy strat = _validateVault(vault);
    ...
    function _validateVault(IPrfctVault _vault) internal view returns (IPrfctStrategy strategy) {
        address vaultAddress = address(_vault);

        try _vault.strategy() returns (IPrfctStrategy _strategy) {
            require(IPrfctStrategy(_strategy).vault() == vaultAddress, "Vault/Strat Mismatch");
            return IPrfctStrategy(_strategy);
        } catch {
            require(false, "Address not a Vault");
        }
    }

    function _collectTokenData(IPrfctStrategy _strategy) internal view returns (address[] memory tokens) {
        ...
        )
    {
        ...
        name_ = vault.name();
        strategy_ = IPrfctStrategy(vault.strategy());
    ...
    function getStakedVaultsForAddress(address _address) external view returns (VaultInfo[] memory stakedVaults) {
        ...
        for (uint256 vid; vid < _vaultSet.length(); vid++) {
            if (IPrfctVault(_vaultSet.at(vid)).balanceOf(_address) > 0) {
                numResults++;
            }
        }
