...
import "../interfaces/IPrfctStrategyEthCall.sol";
...
contract VaultGasOverheadAnalyzer is ManageableUpgradeable, KeeperCompatibleInterface {
    ...
    IPrfctRegistry private _vaultRegistry;
        ...
        )
    {
        ...
        IPrfctVault vaultToAnalyze = IPrfctVault(vaults[currentIndex]);
    ...
    function _analyzeHarvest(IPrfctVault vault) internal view returns (bool didHarvest_, uint256 gasOverhead_) {
        IPrfctStrategyEthCall strategy = IPrfctStrategyEthCall(address(IPrfctVault(vault).strategy()));
    ...
    function _tryHarvest(IPrfctStrategyEthCall strategy) internal view returns (bool didHarvest_, uint256 gasOverhead_) {
    ...
    function _tryOldHarvest(IPrfctStrategyEthCall strategy) internal view returns (bool didHarvest_, uint256 gasOverhead_) {
        uint256 gasBefore = gasleft();
