...
abstract contract BaseAllToNativeStrat is StrategyFeeManagerInitializable {
    ...
    function _swapRewardsToNative() internal virtual {
        for (uint i; i < rewards.length; ++i) {
            ...
            if (amount > minAmounts[token]) {
                IPrfctSwapper(unirouter).swap(token, native, amount);
            }
        }
    }
    ...
    function _swap(address tokenFrom, address tokenTo) internal {
        ...
        IPrfctSwapper(unirouter).swap(tokenFrom, tokenTo, bal);
    }
