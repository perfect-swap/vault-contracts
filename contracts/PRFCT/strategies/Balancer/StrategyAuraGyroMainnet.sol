...
import "./BalancerOrigActionsLib.sol";
import "./PrfctBalancerStructs.sol";
...
contract StrategyAuraGyroMainnet is StrategyFeeManagerInitializable {
    ...
    PrfctBalancerStructs.Input public input;
    ...
    PrfctBalancerStructs.BatchSwapStruct[] public nativeToLp0Route;
    PrfctBalancerStructs.BatchSwapStruct[] public lp0ToLp1Route;
    PrfctBalancerStructs.BatchSwapStruct[] public outputToNativeRoute;
    address[] public nativeToLp0Assets;
    address[] public lp0Tolp1Assets;
    address[] public outputToNativeAssets;

    mapping(address => PrfctBalancerStructs.Reward) public rewards;
    ...
    event ChargedFees(uint256 indexed callFees, uint256 indexed prfctFees, uint256 indexed strategistFees);

    function initialize(
        address _want,
        PrfctBalancerStructs.BatchSwapStruct[] memory _nativeToLp0Route,
        PrfctBalancerStructs.BatchSwapStruct[] memory _lp0ToLp1Route,
        PrfctBalancerStructs.BatchSwapStruct[] memory _outputToNativeRoute,
        address _booster,
        uint256 _pid,
    ...
    function swapRewardsToNative() internal {
        ...
        if (outputBal > 0) {
            IBalancerVault.BatchSwapStep[] memory _swaps = BalancerOrigActionsLib.buildSwapStructArray(outputToNativeRoute, outputBal);
            BalancerOrigActionsLib.balancerSwap(unirouter, swapKind, _swaps, outputToNativeAssets, funds, int256(outputBal));
        }
        // extras
        for (uint i; i < rewardTokens.length; ++i) {
            ...
            if (bal >= rewards[rewardTokens[i]].minAmount) {
                if (rewards[rewardTokens[i]].assets[0] != address(0)) {
                    PrfctBalancerStructs.BatchSwapStruct[] memory swapInfo = new PrfctBalancerStructs.BatchSwapStruct[](rewards[rewardTokens[i]].assets.length - 1);
                    ...
                    IBalancerVault.BatchSwapStep[] memory _swaps = BalancerOrigActionsLib.buildSwapStructArray(swapInfo, bal);
                    BalancerOrigActionsLib.balancerSwap(unirouter, swapKind, _swaps, rewards[rewardTokens[i]].assets, funds, int256(bal));
                } else {
    ...
    function addLiquidity() internal {
        ...
        if (lpTokens[0] != native) {
            IBalancerVault.BatchSwapStep[] memory _swaps = BalancerOrigActionsLib.buildSwapStructArray(nativeToLp0Route, nativeBal);
            BalancerOrigActionsLib.balancerSwap(unirouter, swapKind, _swaps, nativeToLp0Assets, funds, int256(nativeBal));
        }
        ...
        if (nativeBal > 0) {
            ...
            IBalancerVault.BatchSwapStep[] memory _swaps = BalancerOrigActionsLib.buildSwapStructArray(lp0ToLp1Route, lp1Amt);
            BalancerOrigActionsLib.balancerSwap(unirouter, swapKind, _swaps, lp0Tolp1Assets, funds, int256(lp1Amt));
            
            BalancerOrigActionsLib.multiJoin(unirouter, want, poolId, lpTokens[0], lpTokens[1], lp0Amt, IERC20(lpTokens[1]).balanceOf(address(this)));
        }
    }
    ...
    function addRewardToken(address _token, PrfctBalancerStructs.BatchSwapStruct[] memory _swapInfo, address[] memory _assets, bytes calldata _routeToNative, uint _minAmount) external onlyOwner {
