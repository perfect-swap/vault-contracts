...
contract StrategyAuraBalancerMultiRewardGaugeUniV3 is StrategyFeeManagerInitializable {
    ...
    PrfctBalancerStructs.BatchSwapStruct[] public nativeToInputRoute;
    ...
    function initialize(
        ...
        PrfctBalancerStructs.BatchSwapStruct[] memory _nativeToInputRoute,
    ...
    function addLiquidity() internal {
        ...
        if (native != input.input) {
            IBalancerVault.BatchSwapStep[] memory _swaps = BalancerOrigActionsLib.buildSwapStructArray(nativeToInputRoute, nativeBal);
            BalancerOrigActionsLib.balancerSwap(unirouter, swapKind, _swaps, nativeToInputAssets, funds, int256(nativeBal));
        }

        uint256 inputBal = IERC20(input.input).balanceOf(address(this));
        BalancerOrigActionsLib.balancerJoin(unirouter, IBalancerPool(want).getPoolId(), input.input, inputBal);
    }
