...
contract BeTokenBatch is Ownable {
    ...
    constructor(
        address _rewardPool,
        address _unirouter, 
        address _prfctFeeBatch,
        address[] memory _route
    ) {
        rewardPool = IRewardPool(_rewardPool);
        unirouter = IUniswapRouterETH(_unirouter);
        prfctFeeBatch = _prfctFeeBatch;
        route = _route;
    ...
    function harvest() public {
        ...
        if (fee > 0) {
            ...
            unirouter.swapExactTokensForTokens(feeBal, 0, route, prfctFeeBatch, block.timestamp);
        }
    ...
    function setFeeBatch(address _feeBatch) external onlyOwner {
        emit NewFeeBatch(prfctFeeBatch, _feeBatch);
        prfctFeeBatch = _feeBatch;
    }
