...
import { IPrfctSwapper } from "../../interfaces/prfct/IPrfctSwapper.sol";
import { IPrfctRewardPool } from "../../interfaces/prfct/IPrfctRewardPool.sol";
import { StrategyFeeManagerInitializable, IOrigFeeConfig } from "../Common/StrategyFeeManagerInitializable.sol";

/// @title Strategy staking in the PRFCT reward pool
/// @author kexley, Prfct
/// @notice Strategy managing the rewards from the PRFCT reward pool
contract StrategyPrfct is StrategyFeeManagerInitializable {
    using SafeERC20Upgradeable for IERC20Upgradeable;

    /// @notice PRFCT token address
    address public want;
    ...
    /// @notice Reward pool for PRFCT rewards
    ...
    /// @param prfctFees Amount of native sent to the prfct fee recipient
    ...
    function deposit() public whenNotPaused {
        ...
        if (wantBal > 0) {
            IPrfctRewardPool(rewardPool).stake(wantBal);
    ...
    function withdraw(uint256 _amount) external {
        ...
        if (wantBal < _amount) {
            IPrfctRewardPool(rewardPool).withdraw(_amount - wantBal);
    ...
    function _harvest(address _callFeeRecipient) internal whenNotPaused {
        IPrfctRewardPool(rewardPool).getReward();
        _swapToNative();
    ...
    function _swapToNative() internal {
        for (uint i; i < rewards.length; ++i) {
            ...
            if (rewardBal > 0) IPrfctSwapper(unirouter).swap(reward, native, rewardBal);
        }
    }
    ...
    function _chargeFees(address _callFeeRecipient) internal {
        ...
        IERC20Upgradeable(native).safeTransfer(prfctFeeRecipient, prfctFeeAmount);
    ...
    function _swapToWant() internal {
        ...
        if (nativeBal > 0) IPrfctSwapper(unirouter).swap(native, want, nativeBal);
    }
    ...
    function rewardsAvailable() public view returns (uint256 unclaimedReward) {
        unclaimedReward = IPrfctRewardPool(rewardPool).earned(address(this), native);
    }
    ...
    function retireStrat() external {
        ...
        IPrfctRewardPool(rewardPool).withdraw(balanceOfPool());
